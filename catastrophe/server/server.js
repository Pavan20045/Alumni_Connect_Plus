...
// =============================
const app = express();
const port = process.env.PORT || 4000; // ✅ Use dynamic port for Docker/K8s
...
// Prevent multiple initializations (esp. during hot reload)
...
app.post('/alumni/login', async (req, res) => {
  const { username, password } = req.body;
  try {
    const snapshot = await db.ref('alumniUsers').once('value');
    const users = snapshot.val() || {};
...
app.get('/alumni/details', async (req, res) => {
  const { username } = req.query;
  try {
    const snapshot = await db.ref('alumniUsers')
...
// =============================
app.post('/alumni/post-event', upload.single('image'), async (req, res) => {
  const { username, text, expiryDate } = req.body;
  const image = req.file;
...
// ✅ FIX: Explicit wildcard route (prevents path-to-regexp crash)
