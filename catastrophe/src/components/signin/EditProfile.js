...
// src/components/signin/EditProfile.js
import React, { useState, useEffect, useContext } from "react";
import { useLocation, useNavigate } from "react-router-dom";
import "./EditProfile.css";
import { ApiContext } from "../../App"; // ✅ Import centralized API context

function EditProfile() {
  const navigate = useNavigate();
  const { username } = useLocation().state || { username: "User" };

  const api = useContext(ApiContext); // ✅ Use centralized API instance
  ...
  useEffect(() => {
    api
      .get(`/details?username=${username}`)
      .then((res) => setUserData(res.data))
      .catch((err) =>
        alert(err.response?.data?.error || "Error fetching user data")
      );
  }, [api, username]);
  ...
  const handleBack = () => navigate("/profile", { state: { username } });
    ...
    <div className="edit-profile-container">
      ...
      <div className="form-container">
        ...
        <div className="form-row">
          ...
          <button
            className="edit-button"
            onClick={() => handleEditClick("areaOfExpertise")}
          >
            {editableFields.areaOfExpertise ? "Done" : "Edit"}
          </button>
        </div>
        ...
        <div className="form-row">
          ...
          {editableFields.passoutYear ? (
            <input
              ...
              value={userData.passoutYear || ""}
              onChange={(e) =>
                handleChange("passoutYear", e.target.value)
              }
            />
          ) : (
            <span>{userData.passoutYear}</span>
          ...
          <button
            className="edit-button"
            onClick={() => handleEditClick("passoutYear")}
          >
            {editableFields.passoutYear ? "Done" : "Edit"}
          </button>
        </div>
        ...
        <div className="form-row">
          ...
          <button
            className="edit-button"
            onClick={() =>
              handleEditClick("professionalBackground")
            }
          >
            {editableFields.professionalBackground ? "Done" : "Edit"}
          </button>
